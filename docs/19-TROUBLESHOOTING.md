...
| **Source DECIMAL can't be represented exactly** | `errorCode: DECIMAL_TYPE_NOT_SUPPORTED`, non-retryable; `errorMessage` includes `dataset`, `column`, `sourcePrecision`, `sourceScale`, `requestedDuckDbType` | The source Arrow type is always authoritative - the new version's schema is never narrowed toward a previous version's column type. Precision above 38, or scale outside `[0, precision]`, cannot be represented in DuckDB; adjust the source SQL to cast the column. The current ACTIVE version is completely untouched. See [07-REFRESH-AND-VERSIONING.md](07-REFRESH-AND-VERSIONING.md#decimal-schema-evolution) |
